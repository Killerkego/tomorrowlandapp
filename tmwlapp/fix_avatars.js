...
for (const artist in artists) {
    if (artists[artist].includes('d41d8cd98f00b204e9800998ecf8427e')) {
        const randomImage = fallbackImages[Math.floor(Math.random() * fallbackImages.length)];
        artists[artist] = randomImage;
        count++;
        console.log(`Replaced generic Deezer avatar for: ${artist}`);
    }
}

fs.writeFileSync(artistsFile, JSON.stringify(artists, null, 2));
console.log(`Fixed ${count} generic avatars.`);
